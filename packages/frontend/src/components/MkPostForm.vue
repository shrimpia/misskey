...
const submitIcon = computed((): string => {
	return posted.value ? 'ti ti-check' : scheduledAt.value != null ? 'ti ti-calendar-time' : isAirReply ? 'ti ti-bubble-text' : replyTargetNote.value ? 'ti ti-arrow-back-up' : renoteTargetNote.value ? 'ti ti-quote' : 'ti ti-send';
});
...
async function post(ev?: MouseEvent) {
	...
	let postData = {
		...
		replyId: !props.isAirReply && replyTargetNote.value ? replyTargetNote.value.id : undefined,
